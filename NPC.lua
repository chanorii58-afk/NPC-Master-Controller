...
-- Try exploit functions first, but safely
...
hrp.Anchored = false
pcall(function()
hrp.AssemblyLinearVelocity = hrp.AssemblyLinearVelocity + Vector3.new(0, 0.1, 0)
end)

-- If we don't own it yet, briefly teleport to touch it to claim ownership (simulation radius)
-- We do this without relying on isnetworkowner which errors on some executors
local pcallSuccess, age = pcall(function() return hrp.ReceiveAge end)
if pcallSuccess and age ~= 0 and myRoot then
task.spawn(function()
...
-- 1. Clean previous instances safely
...
pcall(function()
    local oldGui = targetParent:FindFirstChild("NPCControllerGUI")
...
BubbleFrame.Position = UDim2.new(0.5, -25, 0.1, 0)
...
CmdsScroll.CanvasSize = UDim2.new(0, 0, 0, 350)
...
if player == LocalPlayer then btnFollow.Text = "Follow Me: OFF" end
    ...
    elseif cmd == "kill" and args[2] then
        ...
        for _, n in ipairs(getNPCs()) do
            if string.lower(n.Name) == args[2] or tostring(n:GetAttribute("NPC_ID")) == args[2] then
                targetNpc = n
                break
            end
        end
...
if pcallSuccess and age ~= 0 then
-- We don't own it, teleport character to it briefly to claim, then teleport it back
local oldCFrame = pRoot.CFrame
local npcStartCFrame = hrp.CFrame

RunService.Stepped:Wait()
pRoot.CFrame = npcStartCFrame

-- Wait a few frames for ownership to transfer
for _ = 1, 3 do RunService.Stepped:Wait() end

-- Force try set ownership if possible
pcall(function()
...
hum.PlatformStand = false
    ...
    if state.Gossip and issuerRoot then
                ...
                if not state.StayingNPCs[npc] then
                    ...
                    if hrp and hum then
                        if math.random() > 0.5 then
                            ...
                            hum:MoveTo(issuerRoot.Position + randomOffset)
                        else
                            -- Do animation
                            local animType = math.random(1, 7)
                            -- 1: disagree (shake head), 2: agreeing (nod), 3: dancing (jump), 4: laughing (spin), 5: scared (shake), 6: normal, 7: crazy (spinning)
                            if animType == 1 then
...
forceConnect(npc)
break -- Only try to force connect one NPC per tick to avoid teleport overlapping
end
end
end
end
...
if hum and hum.Health > 0 then
                if cache and cache.waypoints and cache.currentWaypoint and cache.currentWaypoint <= #cache.waypoints then
                    ...
                    if hrp then
                        ...
                        -- Advanced obstacle jump detection
...
-- 5 NPCs: 2 arms, 2 legs, 1 body. Player is head.
-- We will just use CFrame to position them every frame.
local roles = {
{offset = CFrame.new(-2, -1, 0)}, -- left arm
{offset = CFrame.new(2, -1, 0)}, -- right arm
{offset = CFrame.new(-1, -3, 0)}, -- left leg
{offset = CFrame.new(1, -3, 0)}, -- right leg
{offset = CFrame.new(0, -1, 0.5)}, -- body
}
...
local hrp = npc:FindFirstChild("HumanoidRootPart")
if hum and hrp then
local offset = -3 - (i * 3) -- Line behind the player
                ...
                if npc ~= state.KillTargetNPC and not state.StayingNPCs[npc] then
                    ...
                    if hum and hrp then
                        ...
                        if (hrp.Position - targetRoot.Position).Magnitude < 4 then
                            targetHum.Health = 0 -- Kill on touch
                        end
                    end
                end
            end
...
-- Push the player towards target
...
elseif state.Mode ~= "LookAt" and state.Mode ~= "YesOrNo" then
for _, npc in ipairs(ownedNpcs) do
local hrp = npc:FindFirstChild("HumanoidRootPart")
if hrp then
local gyro = hrp:FindFirstChild("LookAtGyro")
...
-- Clean up any stack physics for staying npcs
local alignPos = hrp:FindFirstChild("StackAlign")
if alignPos then alignPos:Destroy() end
end
end
...
-- Check if direct line of sight is clear
...
if hit and not hit.CanCollide then hit = nil end -- Simplified
...
cache.currentWaypoint = 2 -- Skip first
else
cache.waypoints = nil
end
end)
